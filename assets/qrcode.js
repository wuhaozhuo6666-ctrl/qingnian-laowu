 ...
 * Browser bundle adapted for 青年老吴; source attribution retained below. */
...
function load(name){name=String(name).replace(/^\.\//,'');if(cache[name])return cache[name].exports;if(!modules[name])throw new Error('Unknown QR module '+name);var module={exports:{}};cache[name]=module;modules[name](module,module.exports,load);return module.exports;}
var QRCode=load('index'),Levels=load('QRErrorCorrectLevel');
global.LaowuQR={matrix:function(value,level){var qr=new QRCode(-1,Levels[level]||Levels.M);qr.addData(String(value));qr.make();return qr.modules.map(function(row){return row.map(Boolean);});}};
})(window);
