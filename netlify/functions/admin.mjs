...
const worker = {async fetch(request,env){
 try{
  ...
  if(url.pathname==='/admin/api/product/add'&&request.method==='POST'){
   ...
   const product={id,name,brand:['原木家具','皇玛康之家','文创产品'].includes(brand)?brand:'原木家具',category,room,wood,size,image,real:p.real!==false,options,desc};if(price)product.price=price;
