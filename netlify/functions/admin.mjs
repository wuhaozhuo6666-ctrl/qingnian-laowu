...
const worker = {async fetch(request,env){
 try{
  ...
  if(url.pathname==='/admin/api/product/add'&&request.method==='POST'){
   ...
   const product={id,name,brand:brand==='皇玛康之家'?'皇玛康之家':'原木家具',category,room,wood,size,image,real:p.real!==false,options,desc};if(price)product.price=price;
...
<label><span>产品归属 *</span><select id="newBrand"><option>原木家具</option><option>皇玛康之家</option></select></label>
...
'use strict';const $=id=>document.getElementById(id);const labels={name:'产品名称',brand:'产品归属',category:'产品类型',room:'空间',wood:'木材',size:'尺寸',desc:'产品说明',options:'可定制项目',price:'价格 / 报价说明',image:'产品照片'};const editFields=['brand','name','category','room','wood','size','desc','options','price'];const limits={name:80,brand:20,category:80,room:80,wood:100,size:160,desc:1600,options:400,price:200};let initial=[],draft=[],cases=[],settings={woods:[],categories:[],rooms:[]},sha='',csrf='',busy=false;
...
async function uploadMany(files){const paths=[];for(const file of files){paths.push(await uploadImage(file))}return paths}
