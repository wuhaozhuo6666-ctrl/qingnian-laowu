...
async function readCatalog(token){const file=await github(FILE+'?ref=main',token);const data=JSON.parse(dec.decode(un64(file.content.replace(/\s/g,''))));if(!Array.isArray(data.products))throw Error('Invalid catalog');const settings=data.settings&&typeof data.settings==='object'?data.settings:{};return {sha:file.sha,products:data.products.map(p=>({brand:p.brand||'原木家具',...p})),cases:Array.isArray(data.cases)?data.cases:[],contacts:Array.isArray(data.contacts)?data.contacts:[],settings:{woods:Array.isArray(settings.woods)?settings.woods:[],categories:Array.isArray(settings.categories)?settings.categories:[],rooms:Array.isArray(settings.rooms)?settings.rooms:[]}};}
...
</style></head><body><main><header><div><h1>青年老吴 · 店主管理</h1><div class="hint">产品、照片、案例、联系人和分类都可以单独管理</div></div><a href="https://qingnian-laowu.netlify.app/" target="_blank" rel="noopener">查看顾客网站 ↗</a></header><div id="status" class="status"></div>
...
<nav class="tabs"><button data-tab="products" class="active">产品管理</button><button data-tab="cases">全屋案例</button><button data-tab="contacts">联系我们</button><button data-tab="woods">木材管理</button><button data-tab="categories">产品类型</button><button data-tab="rooms">空间分类</button></nav>
...
async function load(){try{status('正在读取资料…');const d=await api('/admin/api/catalog');csrf=d.csrf;initial=d.products;draft=structuredClone(initial);cases=d.cases||[];contacts=d.contacts||[];settings=d.settings||{woods:[],categories:[],rooms:[]};sha=d.sha;render();renderCasesAdmin();renderContactsAdmin();renderSettings();$('editor').hidden=false;$('loginBox').hidden=true;status('资料已加载。')}catch(e){status(e.message)}}
document.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-tab]').forEach(x=>x.classList.toggle('active',x===b));for(const name of ['products','cases','contacts','woods','categories','rooms'])$('tab-'+name).hidden=b.dataset.tab!==name});
