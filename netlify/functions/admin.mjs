...
<nav class="tabs"><button data-tab="products" class="active">产品管理</button><button data-tab="woods">木材管理</button><button data-tab="categories">产品类型</button><button data-tab="rooms">空间分类</button></nav>
...
<label class="wide"><span>产品照片 *</span><input id="newImage" type="file" accept="image/*"></label>
</div><p class="hint">支持常见 JPG / PNG / WEBP。若电脑里的 HEIC 原图无法读取，请先转成 JPG。</p><div class="toolbar"><button id="createProduct" class="primary">添加产品</button><button id="cancelAdd">取消</button></div></section>
...
'use strict';const $=id=>document.getElementById(id);const labels={name:'产品名称',category:'产品类型',room:'空间',wood:'木材',size:'尺寸',desc:'产品说明',options:'可定制项目',price:'价格 / 报价说明',image:'产品照片'};const editFields=['name','category','room','wood','size','desc','options','price'];const limits={name:80,category:80,room:80,wood:100,size:160,desc:1600,options:400,price:200};let initial=[],draft=[],settings={woods:[],categories:[],rooms:[]},sha='',csrf='',busy=false;
function el(t,x){const e=document.createElement(t);if(x!==undefined)e.textContent=x;return e}function status(x){$('status').textContent=x}function setBusy(v){busy=v;document.querySelectorAll('button,input,textarea').forEach(e=>e.disabled=v)}
...
function changes(){return draft.flatMap(p=>{const o=initial.find(x=>x.id===p.id);if(!o)return[];const f={};for(const k of Object.keys(labels))if((p[k]||'')!==(o[k]||''))f[k]=p[k]||'';return Object.keys(f).length?[{id:p.id,fields:f}]:[]})}
...
function render(){fillLists();const q=$('search').value.trim().toLowerCase();const list=draft.filter(p=>[p.id,p.name,p.category,p.room,p.wood].join(' ').toLowerCase().includes(q));$('products').replaceChildren();$('count').textContent=list.length+' 个产品';for(const p of list){const card=el('article');card.className='product';const left=el('div');const img=el('img');if(p.image)img.src='https://qingnian-laowu.netlify.app/'+p.image;left.append(img,el('div',p.id));const pa=el('div');pa.className='photoActions';const up=el('input');up.type='file';up.accept='image/*';up.onchange=async()=>{const file=up.files&&up.files[0];if(!file)return;try{setBusy(true);p.image=await uploadImage(file);status('新照片已上传。点击“预览并发布修改”后顾客网站会改用这张照片。')}catch(e){status(e.message)}finally{setBusy(false)}};const del=el('button','删除产品');del.className='danger';del.onclick=async()=>{if(!confirm('确定删除“'+p.name+'”？'))return;try{setBusy(true);const d=await api('/admin/api/product/delete',{method:'POST',body:JSON.stringify({sha,id:p.id})});initial=d.products;draft=structuredClone(initial);settings=d.settings||settings;sha=d.sha;render();renderSettings();status(d.message)}catch(e){status(e.message)}finally{setBusy(false)}};pa.append(up,del);left.append(pa);const fields=el('div');fields.className='fields';for(const k of editFields){const wrap=el('label');if(['desc','options','price'].includes(k))wrap.className='wide';wrap.append(el('span',labels[k]));const input=el(k==='desc'?'textarea':'input');input.maxLength=limits[k];if(k==='wood')input.setAttribute('list','woodList');if(k==='category')input.setAttribute('list','categoryList');if(k==='room')input.setAttribute('list','roomList');input.value=p[k]||'';input.oninput=()=>{p[k]=input.value;$('previewPanel').hidden=true;status('有 '+changes().length+' 个产品修改尚未发布。')};wrap.append(input);fields.append(wrap)}card.append(left,fields);$('products').append(card)}}
async function load(){try{status('正在读取资料…');const d=await api('/admin/api/catalog');csrf=d.csrf;initial=d.products;draft=structuredClone(initial);settings=d.settings||{woods:[],categories:[],rooms:[]};sha=d.sha;render();renderSettings();$('editor').hidden=false;$('loginBox').hidden=true;status('资料已加载。')}catch(e){status(e.message)}}
document.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-tab]').forEach(x=>x.classList.toggle('active',x===b));for(const name of ['products','woods','categories','rooms'])$('tab-'+name).hidden=b.dataset.tab!==name});
...
$('createProduct').onclick=async()=>{const file=$('newImage').files&&$('newImage').files[0];const p={name:$('newName').value.trim(),category:$('newCategory').value.trim(),room:$('newRoom').value.trim()||'其他',wood:$('newWood').value.trim(),size:$('newSize').value.trim(),price:$('newPrice').value.trim(),desc:$('newDesc').value.trim(),options:$('newOptions').value.trim(),real:true};if(!p.name||!p.category||!p.wood||!file){status('请填写产品名称、产品类型、木材，并选择照片。');return}try{setBusy(true);p.image=await uploadImage(file);const d=await api('/admin/api/product/add',{method:'POST',body:JSON.stringify({sha,product:p})});initial=d.products;draft=structuredClone(initial);settings=d.settings||settings;sha=d.sha;['newName','newCategory','newRoom','newWood','newSize','newPrice','newDesc','newOptions'].forEach(id=>$(id).value='');$('newImage').value='';$('addPanel').hidden=true;render();renderSettings();status(d.message)}catch(e){status(e.message)}finally{setBusy(false)}};
$('preview').onclick=()=>{const edits=changes();if(!edits.length){status('还没有修改。');return}$('changes').replaceChildren(...edits.map(ch=>{const d=el('div');d.append(el('strong',ch.id));for(const [k,v]of Object.entries(ch.fields))d.append(el('p',labels[k]+'：'+v));return d}));$('previewPanel').hidden=false};
