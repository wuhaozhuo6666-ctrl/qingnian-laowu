...
const LIMITS = {name:80,brand:20,category:80,room:80,wood:100,size:160,desc:1600,options:400,price:200,image:260,title:80,phone:80,wechat:80,bio:500};
...
async function readCatalog(token){const file=await github(FILE+'?ref=main',token);const data=JSON.parse(dec.decode(un64(file.content.replace(/\s/g,''))));if(!Array.isArray(data.products))throw Error('Invalid catalog');const settings=data.settings&&typeof data.settings==='object'?data.settings:{};return {sha:file.sha,products:data.products.map(p=>({brand:p.brand||'原木家具',...p})),cases:Array.isArray(data.cases)?data.cases:[],contacts:Array.isArray(data.contacts)?data.contacts:[],settings:{heroImage:validImagePath(settings.heroImage)?settings.heroImage:'products/showroom/home-hero.webp',woods:Array.isArray(settings.woods)?settings.woods:[],categories:Array.isArray(settings.categories)?settings.categories:[],rooms:Array.isArray(settings.rooms)?settings.rooms:[]}};}
...
<nav class="tabs"><button data-tab="products" class="active">产品管理</button><button data-tab="homepage">首页大图</button><button data-tab="cases">全屋案例</button><button data-tab="contacts">联系我们</button><button data-tab="woods">木材管理</button><button data-tab="categories">产品类型</button><button data-tab="rooms">空间分类</button></nav>
...
function renderSettings(){fillLists();renderChips('woodChips','woods');renderChips('categoryChips','categories');renderChips('roomChips','rooms');const hero=settings.heroImage||'products/showroom/home-hero.webp';$('heroPreview').src=imageSrc(hero);$('heroPath').textContent=hero}
...
$('saveHeroImage').onclick=async()=>{const file=$('heroImageInput').files&&$('heroImageInput').files[0];if(!file){status('请先选择一张新的首页大图。');return}try{setBusy(true);const heroImage=await uploadImage(file);const d=await api('/admin/api/settings',{method:'POST',body:JSON.stringify({sha,settings:{...settings,heroImage}})});sha=d.sha;settings=d.settings;$('heroImageInput').value='';renderSettings();status('首页大图已更换，顾客刷新网站即可看到。')}catch(e){status(e.message)}finally{setBusy(false)}};
function render(){
...
async function load(){try{status('正在读取资料…');const d=await api('/admin/api/catalog');csrf=d.csrf;initial=d.products;draft=structuredClone(initial);cases=d.cases||[];contacts=d.contacts||[];settings=d.settings||{heroImage:'products/showroom/home-hero.webp',woods:[],categories:[],rooms:[]};sha=d.sha;render();renderCasesAdmin();renderContactsAdmin();renderSettings();$('editor').hidden=false;$('loginBox').hidden=true;status('资料已加载。')}catch(e){status(e.message)}}
document.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-tab]').forEach(x=>x.classList.toggle('active',x===b));for(const name of ['products','homepage','cases','contacts','woods','categories','rooms'])$('tab-'+name).hidden=b.dataset.tab!==name});
...
function renderContactsAdmin(){const box=$('contactList');box.replaceChildren(...contacts.map(c=>{const card=el('article');card.className='product';const left=el('div');const img=el('img');if(c.image)img.src=imageSrc(c.image);img.alt=c.name+'个人照片';left.append(img,el('div',c.id));const right=el('div');right.append(el('h2',(c.type==='designer'?'设计师 · ':'销售 · ')+c.name),el('p',c.title||'未填写职位'),el('p','电话：'+(c.phone||'未填写')+'　微信：'+(c.wechat||'未填写')));if(c.bio)right.append(el('p',c.bio));const del=el('button','删除联系人');del.className='danger';del.onclick=async()=>{if(!confirm('确定删除“'+c.name+'”？'))return;try{setBusy(true);const d=await api('/admin/api/contact/delete',{method:'POST',body:JSON.stringify({sha,id:c.id})});contacts=d.contacts||[];sha=d.sha;renderContactsAdmin();status(d.message)}catch(e){status(e.message)}finally{setBusy(false)}};right.append(del);card.append(left,right);return card}))}
...
$('addContact').onclick=async()=>{const image=$('contactImage').files&&$('contactImage').files[0],name=$('contactName').value.trim(),phone=$('contactPhone').value.trim(),wechat=$('contactWechat').value.trim();if(!name||!image||(!phone&&!wechat)){status('请填写姓名和至少一种联系方式，并选择个人照片。');return}try{setBusy(true);const imagePath=await uploadImage(image);const contact={type:$('contactType').value,name,title:$('contactTitle').value.trim(),phone,wechat,bio:$('contactBio').value.trim(),image:imagePath};const d=await api('/admin/api/contact/add',{method:'POST',body:JSON.stringify({sha,contact})});contacts=d.contacts||[];sha=d.sha;['contactName','contactTitle','contactPhone','contactWechat','contactBio'].forEach(id=>$(id).value='');$('contactImage').value='';renderContactsAdmin();status(d.message)}catch(e){status(e.message)}finally{setBusy(false)}};
